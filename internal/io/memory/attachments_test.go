...
func TestAttachmentRoundTrip(t *testing.T) {
	...
	// 返回的是拷贝:改它不应污染存储。
	got.Bytes[0] = 0
	again, _ := store.GetAttachment(ctx, "room-a", "att-1")
	if again.Bytes[0] != 0x89 {
		t.Fatal("GetAttachment returned aliased bytes")
	}
...
func TestAttachmentRoomUsageAndDelete(t *testing.T) {
	...
	for _, att := range []models.Attachment{
		{ID: "a1", RoomID: "room-a", MIME: "image/png", Size: 3, Bytes: []byte("aaa")},
		{ID: "a2", RoomID: "room-a", MIME: "image/jpeg", Size: 5, Bytes: []byte("bbbbb")},
		{ID: "b1", RoomID: "room-b", MIME: "image/png", Size: 7, Bytes: []byte("ccccccc")},
	} {
		if err := store.PutAttachment(ctx, att); err != nil {
